...
class OvertureProhibitedTransitionTest {
    ...
    void testConstructorAndGetters() {
        OvertureProhibitedTransition transition = new OvertureProhibitedTransition(sequence, heading, when, between);
    ...
    void testEqualsAndHashCode() {
        OvertureProhibitedTransition t1 = new OvertureProhibitedTransition(sequence, heading, when, between);
        OvertureProhibitedTransition t2 = new OvertureProhibitedTransition(sequence, heading, when, between);
    ...
    void testInequalityAcrossAllFields() {
        OvertureProhibitedTransition base = new OvertureProhibitedTransition(sequence, heading, when, between);

        assertAll("Field-by-field inequality",
                () -> assertNotEquals(base, new OvertureProhibitedTransition(List.of(), heading, when, between)),
                () -> assertNotEquals(base, new OvertureProhibitedTransition(sequence, TravelHeading.FORWARD, when, between)),
                () -> assertNotEquals(base, new OvertureProhibitedTransition(sequence, heading, null, between)),
                () -> assertNotEquals(base, new OvertureProhibitedTransition(sequence, heading, when, new LinearlyReferencedRange(0, 1)))
        );
    }
